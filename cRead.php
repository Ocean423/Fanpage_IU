...
<div class="w3-center w3-dark-gray w3-container w3-padding-64">
    <div class="wbox" >
        <?php
        $no = $_GET['no'];
		$sql = "select * from community where no = '$no'";
		$result = $conn->query($sql);
		if($result->num_rows > 0){
			while($row=$result->fetch_assoc()){
		?>
        
        <h2><?=$row['writer']?>님의 글</h2>
        <form action="" method="">
            
            <label >제목</label>
            <input class="title" type="text" name="title" value="<?=$row['title']?>"><br>
            <label >내용</label>
            <input class="contents" type="text" name="contents" value="<?=$row['content']?>"><br>
            <hr>
            <button type="button" class="btn" onclick="location.href='community.php'">게시판으로 돌아가기</button>
            <button type="button" class="btn" onclick="location.href='addCont.php'">새 게시글 작성</button>
        </form>
        <?php } ?>
        <?php } ?>
    </div>
